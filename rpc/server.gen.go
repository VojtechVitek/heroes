...
func (s *aPIServer) serveGetMapJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	func() {
		...
		ret0, err = s.API.GetMap(ctx, reqContent.Arg0)
	}()
	respContent := struct {
		Ret0 *Map `json:"m"`
	}{ret0}
...
func (s *aPIServer) serveListMaps(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	header := r.Header.Get("Content-Type")
	i := strings.Index(header, ";")
	if i == -1 {
		i = len(header)
...
func (s *aPIServer) serveListMapsJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	var err error
