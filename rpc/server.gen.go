...
//
// webrpc-gen -schema=./rpc/webrpc.json -target=../../webrpc/gen-golang -pkg=rpc -server -types=false -out=./rpc/server.gen.go
package rpc
...
type heroesServerServer struct {
	HeroesServer
}

func NewHeroesServerServer(svc HeroesServer) WebRPCServer {
	return &heroesServerServer{
		HeroesServer: svc,
	}
}

func (s *heroesServerServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	ctx = context.WithValue(ctx, HTTPResponseWriterCtxKey, w)
	ctx = context.WithValue(ctx, HTTPRequestCtxKey, r)
	ctx = context.WithValue(ctx, ServiceNameCtxKey, "HeroesServer")
	...
	switch r.URL.Path {
	case "/rpc/HeroesServer/GetMap":
		s.serveGetMap(ctx, w, r)
		return
	case "/rpc/HeroesServer/ListMaps":
		s.serveListMaps(ctx, w, r)
		return
	default:
...
func (s *heroesServerServer) serveGetMap(ctx context.Context, w http.ResponseWriter, r *http.Request) {
...
func (s *heroesServerServer) serveGetMapJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	var err error
	...
	func() {
		...
		ret0, err = s.HeroesServer.GetMap(ctx, reqContent.Arg0)
	}()
	respContent := struct {
		Ret0 *Map `json:"m"`
	}{ret0}
...
func (s *heroesServerServer) serveListMaps(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	header := r.Header.Get("Content-Type")
	i := strings.Index(header, ";")
	if i == -1 {
		i = len(header)
...
func (s *heroesServerServer) serveListMapsJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	var err error
	ctx = context.WithValue(ctx, MethodNameCtxKey, "ListMaps")
	...
	func() {
		...
		ret0, err = s.HeroesServer.ListMaps(ctx)
	}()
	respContent := struct {
